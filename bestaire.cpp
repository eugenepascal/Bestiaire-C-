...
void afficheBestiaire(EspecePokemon bestiaire[]) {
    ...
    for (int i = 0; i < 150; i++) {
        ...
        if (bestiaire[i].evolvesTo != nullptr) {
            cout << " - Devient " << bestiaire[i].evolvesTo << " pour " << bestiaire[i].nbBonbonsPourEvoluer << " bonbons. ";
        }
        cout << endl;
    }
}
...
void afficheBestiaireTrie(EspecePokemon bestiaire[]) {
    ...
    // nous voulons conserver le bestiaire tel que construit.
    // nous construisons une copie du tableau du bestiaire afin de réaliser notre tri.
    // puis nous pouvons appeler la fonction d'affichage d'un bestiaire avec ce nouveau tableau trié par nom.

    // 1- copie dans un nouveau tableau
    ...
    // 2- tri par sélection
    for (int i = 0; i < 150; i++) {
        char* minStr = bestiaireTrie[i].nom;
        int minPos = i;
    ...
    // 3- affichage en réutilisant la fonction existante.
    afficheBestiaire(bestiaireTrie);
}

// Rechercher un pokemon dans le bestiaire.
EspecePokemon lookupEspecePokemon(EspecePokemon bestiaire[], char* nom) {
    ...
    // si l'on n'a pas trouvé l'espèce recherchée, on renvoie une espèce fictive
    EspecePokemon e;
    e.nom = "UNKNOWN";
    e.type = Normal;
